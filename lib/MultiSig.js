...
module.exports = {
    ...
    addOwnerMultiSig: function (contract, newOwner){
        ...
        return new Promise((resolve, reject) => {
            const ownerData = contract.addOwner.getData(newOwner);
    ...
    removeOwnerMultiSig: function(contract, oldOwner){
        var options = { from: web3.eth.accounts[0], gas: 2000000 }
        return new Promise((resolve, reject) => {
            const ownerData = contract.removeOwner.getData(oldOwner);
            var event = contract.Confirmation();
            event.watch(function(err, result){
                if(err) reject(err)
                else resolve(result.args.transactionId);
    ...
    replaceOwnerMultiSig: function(contract, oldOwner, newOwner){
        var options = { from: web3.eth.accounts[0], gas: 2000000 }
        return new Promise((resolve, reject) => {
            const ownerData = contract.replaceOwner.getData(oldOwner, newOwner);
            var event = contract.Confirmation();
            event.watch(function(err, result){
                if(err) reject(err)
                else resolve(result.args.transactionId);
