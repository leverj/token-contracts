...
describe('sale tests', ()=>{
	...
	describe('sale end-to-end test', () => {
			...
			var testSale;
			var owner = "0x4458ba7a281f4fe0359a4a57c59d2af72ad53c36";
...
describe('test post configuration phases', ()=>{

	describe('repeat configuration steps in minimal fashion to setup for post-config tests :D ', () => {
		...
		describe('creation of sale contract with block windows for testing full process', () => {
			...
			it('sale should have the correct parameters saved', async () => {
				...
				owner1 = web3.eth.accounts[0];
				freezeBlock1 = web3.eth.blockNumber + 25; //tweak this based on how long it takes for configuration to complete
				startBlock1 = freezeBlock1 + 10; //tweak this based on how long it takes freeze tests to complete
				endBlock1 = startBlock1 + 10; //tweak this based on how long it takes purchases simulation to complete
		...
		describe('distribute private sale tokens', () => {
			...
			const timeNow = Math.floor(Date.now() / 1000);
			const timelocks = [timeNow + 300, timeNow + 300, timeNow + 300];
			const durations = [300, 50000, 259200];
			...
			it('should have an address for disbursements ', async ()=> {
				let disbursement1= testSale1.disbursements(0);
				let disbursement2 = testSale1.disbursements(1);
				let disbursement3 = testSale1.disbursements(2);
				assert(JSON.parse(web3.isAddress(disbursement1)) === true, 'disbursement contract at index 0 does not have an address');
				assert(JSON.parse(web3.isAddress(disbursement2)) === true, 'disbursement contract at index 1 does not have an address');
				assert(JSON.parse(web3.isAddress(disbursement3)) === true, 'disbursement contract at index 2 does not have an address');
			}).timeout(300000);
			...
			it('should have the correct properties on disbursements', async ()=> {
				...
				let disbursementAddress1 = testSale1.disbursements(0);
				let disbursement1 = await Disbursement.at(disbursementAddress1);
				let setOwner1 = 	disbursement1.owner();
				let	receiver1 = disbursement1.receiver();
				let	disbursementPeriod1 = disbursement1.disbursementPeriod();
				...
				let	withdrawnTokens1 = disbursement1.withdrawnTokens();
				let token1 =	disbursement1.token();
				assert(setOwner1 === testSale1.address, 'disbursement does not have the right owner');
				assert(receiver1 === beneficiaries[0], 'disbursement does not have the right owner');
				assert(JSON.parse(disbursementPeriod1) === durations[0], 'disbursement does not have the right owner');
				assert(JSON.parse(startDate1) === timelocks[0], 'disbursement does not have the right owner');
				assert(JSON.parse(withdrawnTokens1) === 0, 'disbursement does not have the right owner');
				assert(token1 === testSale1.token(), 'disbursement does not have the right owner');

				let disbursementAddress2 = testSale1.disbursements(1);
				let disbursement2 = await Disbursement.at(disbursementAddress2);
				let setOwner2 = 	disbursement2.owner();
				let	receiver2 = disbursement2.receiver();
				let	disbursementPeriod2 = disbursement2.disbursementPeriod();
				...
				let	withdrawnTokens2 = disbursement2.withdrawnTokens();
				let token2 =	disbursement2.token();
				assert(setOwner2 === testSale1.address, 'disbursement does not have the right owner');
				assert(receiver2 === beneficiaries[1], 'disbursement does not have the right owner');
				assert(JSON.parse(disbursementPeriod2) === durations[1], 'disbursement does not have the right owner');
				assert(JSON.parse(startDate2) === timelocks[1], 'disbursement does not have the right owner');
				assert(JSON.parse(withdrawnTokens2) === 0, 'disbursement does not have the right owner');
				assert(token2 === testSale1.token(), 'disbursement does not have the right owner');
		...
		describe('set wallet', ()=> {
			it('trying to set the wallet as the owner should finish without errors', async () => {
				let txHash = await sale.configureWallet(testSale1.address, web3.eth.accounts[7]);
				let newWallet = await testSale1.wallet();
		...
		describe('add whitelist entries', () => {
			...
			it('whitelist entry should be processed without failure', async () => {
				let txHash = await sale.addWhitelist(testSale1.address, addresses2, amounts2);
				for(var i = 0; i < addresses2.length; i++){
					let whitelistedAmount = await testSale1.whitelistRegistrants(addresses2[i]);
					assert(JSON.parse(whitelistedAmount) === amounts2[i], 'this address amount was not set');
				}
				let newAddresses = [web3.eth.accounts[18]];
				let newAmounts = [10];
				let txHash1 = await sale.addWhitelist(testSale1.address, newAddresses, newAmounts);
				let whitelistedAmount1 = await testSale1.whitelistRegistrants(web3.eth.accounts[18]);
				assert(JSON.parse(whitelistedAmount1) === 10, 'this address amount was not set');
			}).timeout(3000000);

		}).timeout(3000000);

		describe('mark setup complete', async ()=>{
			...
			it('should have the right properties before the sale starts', async ()=> {
				let tokenAddress = await testSale1.token();
				let token = await HumanStandardToken.at(tokenAddress);
				var totalPrivateSold = testSale1.privateAllocated();
				var tokenBalanceOfSale = token.balanceOf(testSale1.address);
				var balanceOfWallet = web3.eth.getBalance(testSale1.wallet());
				console.log("token balance of wallet before sale: " + JSON.parse(balanceOfWallet));
			...
			it('post config tests', async ()=>{
				let freezeTestsDone = false;
				let startTestsDone = false;
				let endTestsDone = false;

				while(!(endTestsDone && startTestsDone && freezeTestsDone)){
				
					describe('test describe', async ()=>{

					console.log("freezeblock: " + freezeBlock1)
					if((web3.eth.blockNumber >= freezeBlock1) && !freezeTestsDone){

						describe('carry out freeze period tests', async ()=>{

							it('should not allow anyone to change the start block once the sale is frozen', async ()=>{
									let existingStartBlock = testSale1.startBlock();
									let txHash = sale.changeStartBlock(testSale1.address, existingStartBlock-1);
									let newStartBlock = testSale1.startBlock();
							...
							it('should not allow a valid change price once the sale is frozen', async ()=>{
									let existingPrice = testSale1.price_in_wei();
									let txHash = sale.changePrice(testSale1.address, 222222);
									let newPrice = testSale1.price_in_wei();
							...
							it('should not allow anyone to purchase tokens before the sale starts', async ()=> {
									...
									var txHash = await sale.purchaseTokens(testSale1.address, web3.eth.accounts[11], Math.floor((Math.random() * 500) + 1));
						...
						freezeTestsDone = true;

						}).timeout(300000);
							

					}else if((web3.eth.blockNumber >= startBlock1) && !startTestsDone){
						describe('carry out start period tests', async ()=>{

							it('should allow anyone to purchase tokens now that the sale has started', async ()=>{
								console.log("buyer eth balance before: " + web3.eth.getBalance(web3.eth.accounts[11]));
								let tokenAddress = await testSale1.token();
								let token = await HumanStandardToken.at(tokenAddress);
								let buyerBalanceBefore = token.balanceOf(web3.eth.accounts[11]);
								var weiToSend = Math.floor((Math.random() * 51234532467654345675) + 51234567654345675);
								console.log("wei to send: " + weiToSend);
								var txHash = await sale.purchaseTokens(testSale1.address, web3.eth.accounts[11], weiToSend);
								let buyerBalanceAfter = token.balanceOf(web3.eth.accounts[11]);
								console.log("tokens bought: " + buyerBalanceAfter);
								console.log("buyer eth balance after: " + web3.eth.getBalance(web3.eth.accounts[11]));
							...
							it('should not allow someone that hasnt been whitelisted to purchase any tickets', async ()=>{
								...
								console.log("wei to send: " + weiToSend);
								var txHash = await sale.purchaseTokens(testSale1.address, web3.eth.accounts[19], weiToSend);
								let buyerBalanceAfter = token.balanceOf(web3.eth.accounts[19]);
								console.log("tokens bought: " + buyerBalanceAfter);
								assert(JSON.parse(buyerBalanceBefore) == JSON.parse(buyerBalanceAfter), 'buyer should have been able to get tokens');
							}).timeout(300000);

							//TOOD: cant call lock unsold before sale ended

							it('should allow anyone to buy their full whitelist cap in one shot', async ()=>{
								let whitelistedAmount1 = await testSale1.whitelistRegistrants(web3.eth.accounts[18]);
								console.log("whitelisted amount: "+ JSON.parse(whitelistedAmount1));
								let tokenAddress = await testSale1.token();
								let token = await HumanStandardToken.at(tokenAddress);
								let buyerBalanceBefore = token.balanceOf(web3.eth.accounts[18]);
								var txHash = await sale.purchaseTokens(testSale1.address, web3.eth.accounts[18], 223423429842934723);
								let buyerBalanceAfter = token.balanceOf(web3.eth.accounts[18]);
								console.log("tokens bought buyer 2: " + JSON.parse(buyerBalanceAfter));
								assert(JSON.parse(buyerBalanceBefore) != JSON.parse(buyerBalanceAfter), 'buyer should have been able to get tokens');
							}).timeout(300000);
							...
							it('should allow repeated purchases until the cap is hit', async ()=>{
								...
								while(JSON.parse(token.balanceOf(testSale1.address) > 0 )){
									...
									var weiToSend = Math.floor((Math.random() * 50000000000000000000000) + 50000000000000000000);
									console.log("wei to send: " + weiToSend);
									var txHash = await sale.purchaseTokens(testSale1.address, web3.eth.accounts[randomBuyerIndex], weiToSend);
									let buyerBalanceAfter = token.balanceOf(web3.eth.accounts[randomBuyerIndex]);
									console.log("tokens bought: " + buyerBalanceAfter);
									assert(JSON.parse(buyerBalanceBefore) != JSON.parse(buyerBalanceAfter), 'buyer should have been able to get tokens');
								}

								let newSaleBalance = token.balanceOf(testSale1.address);
								assert(JSON.parse(newSaleBalance) === 0, 'the sale contract should not have a balance anymore');
							}).timeout(300000);

							it('should not let you buy if it is sold out', async ()=>{
								let tokenAddress = await testSale1.token();
								let token = await HumanStandardToken.at(tokenAddress);
								let buyerBalanceBefore = token.balanceOf(web3.eth.accounts[11]);
							...
							*/
							...
							it('should not let you buy if the emergency toggle is set', async ()=>{
								let txHash1 = await sale.emergencyToggle(testSale1.address);
								let tokenAddress = await testSale1.token();
								let token = await HumanStandardToken.at(tokenAddress);
								let buyerBalanceBefore = token.balanceOf(web3.eth.accounts[11]);
								var weiToSend = 10*333333;
								...
								let txHash2 = await sale.emergencyToggle(testSale1.address); //lets turn it back off for remaining tests
							}).timeout(300000);
					...
					}else if((web3.eth.blockNumber >= endBlock1) && !endTestsDone){
						describe('carry out end period tests', async ()=>{
							...
							it('should not allow a non-owner to call the reversal function', async ()=>{
								...
								let txHash = await sale.reversePurchaseAs(web3.eth.accounts[1], testSale1.address, web3.eth.accounts[11], wei);
							...
							it('should not reverse tokens if inadequate ether is sent', async ()=>{
								...
								const previousBalance = token.balanceOf(web3.eth.accounts[11]);
								assert(JSON.parse(previousBalance) != 0, 'cant do test if old balance is already 0');
								let txHash = await sale.reversePurchase(testSale1.address, web3.eth.accounts[11], 5000000000000000000);
								const newBalance = token.balanceOf(web3.eth.accounts[11]);
								assert(JSON.parse(newBalance) === JSON.parse(previousBalance), 'tokens should not have been reversed');
								assert(JSON.parse(newBalance) != 0, 'tokens should not have been reversed');
							}).timeout(3000000);
							...
							it('should reverse tokens and allocate funds if valid reversal', async ()=>{
								...
								assert(JSON.parse(previousBalance) != 0, 'cant do test if old balance is already 0');
								const wei = (JSON.parse(previousBalance) * 333333) + 3000000000000;
								let txHash = await sale.reversePurchase(testSale1.address, web3.eth.accounts[11], wei);
								const newBalance = token.balanceOf(web3.eth.accounts[11]);
							...
							it('should not allow a non-owner to call the transfer lock removal', async ()=> {
								let tokenAddress = await testSale1.token();
								let token = await HumanStandardToken.at(tokenAddress);
								let txHash = await sale.removeTransferLockAs(web3.eth.accounts[1], testSale1.address);
							...
							it('should allow an owner to call the transfer lock removal', async ()=> {
								let tokenAddress = await testSale1.token();
								let token = await HumanStandardToken.at(tokenAddress);
								let txHash = await sale.removeTransferLock(testSale1.address);
								const flag = token.transfersAllowed();
							...
							it('should not reverse if transfer lock has already been removed', async ()=>{
								let tokenAddress = await testSale1.token();
								let token = await HumanStandardToken.at(tokenAddress);
								const previousBalance = token.balanceOf(web3.eth.accounts[12]);
								assert(JSON.parse(previousBalance) != 0, 'cant do test if old balance is already 0');
								let txHash = await sale.reversePurchase(testSale1.address, web3.eth.accounts[12], 500000000000000000000);
								const newBalance = token.balanceOf(web3.eth.accounts[12]);
								assert(JSON.parse(newBalance) === JSON.parse(previousBalance), 'tokens should not have been reversed');
								assert(JSON.parse(newBalance) != 0, 'tokens should not have been reversed');
							}).timeout(3000000);
							...
							it('should not allow a non-owner to call lock unsold', async ()=>{
								let tokenAddress = await testSale1.token();
								let token = await HumanStandardToken.at(tokenAddress);
								let oldBalance = token.balanceOf(testSale1.address);
								let txHash = await sale.lockUnsoldTokens(web3.eth.accounts[1], testSale1.address, web3.eth.accounts[19]);
								let newBalance = token.balanceOf(testSale1.address);
							...
							it('should not allow an invalid address for lock unsold', async ()=>{
								...
								let token = await HumanStandardToken.at(tokenAddress);
								let oldBalance = token.balanceOf(testSale1.address);
								let txHash = await sale.lockUnsoldTokens(web3.eth.accounts[0], testSale1.address, 0);
								let newBalance = token.balanceOf(testSale1.address);
								assert(JSON.parse(oldBalance) === JSON.parse(newBalance), 'should not have moved unsold tokens');
							}).timeout(3000000);
							...
							it('a valid address can call lock unsold', async ()=>{
								...
								let token = await HumanStandardToken.at(tokenAddress);
								let oldBalance = token.balanceOf(testSale1.address);
								let txHash = await sale.lockUnsoldTokens(web3.eth.accounts[0], testSale1.address, web3.eth.accounts[19]);
								let newBalance = token.balanceOf(testSale1.address);
								assert(JSON.parse(oldBalance) > JSON.parse(newBalance), 'should not have moved unsold tokens');
							}).timeout(3000000);
...
/* botched approach to triggering tests of phase change blocks ...

var saleWatcher = web3.eth.filter('latest');
saleWatcher.watch(function(error, result){
	if (!error){
		blockNumber = web3.eth.getBlock(result).number;
		console.log("new block: " + blockNumber);
		switch(blockNumber) {
			case freezeBlock1:
				saleFrozen = true;
				testOnFrozen(testSale1, assert);
				break;
			case startBlock1-5:
			...
			case endBlock1:
				saleEnded = true;
				testsOnSaleEnd(testSale1);
			default:
		} 
	}
});

function testOnFrozen(saleToTest){
	describe('freeze period specific tests', ()=>{
	}).timeout(3000000);
}

function testsOnSaleStart(saleToTest){
	describe('start period specific tests', ()=>{
	}).timeout(3000000);
}

function testsOnSaleEnd(saleToTest){
	describe('start period specific tests', ()=>{
	}).timeout(3000000);
}
*/